...
class IntermediateFusionDatasetV5(basedataset.BaseDataset):
    """
    This class is for intermediate fusion with mulit agent (more than two)
    ego agent use preprocess_q
    other agents ues preprocess_k
    """

    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetV5, self).__init__(params, visualize, train)
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            if cav_id == ego_id:
                selected_cav_processed = self.get_item_single_car_v2(
                    selected_cav_base, ego_lidar_pose, isEgo=True
                )
                processed_features_ego.append(selected_cav_processed["processed_features"])
            else:
                selected_cav_processed = self.get_item_single_car_v2(
                    selected_cav_base, ego_lidar_pose, isEgo=False
                )
                processed_features_stack.append(selected_cav_processed["processed_features"])
        ...
        merged_feature_ego_dict=self.merge_features_to_dict(processed_features_ego)
        cav_num = len(processed_features_stack)+1
        if len(processed_features_stack)==0:
            merged_feature_nig_dict = merged_feature_ego_dict.copy()
            cav_num+=1
        else:
            merged_feature_nig_dict = self.merge_features_to_dict(processed_features_stack)
        ...
        processed_data_dict["ego"].update(
            {
                ...
                "processed_lidar_q": merged_feature_ego_dict,
                "processed_lidar_k": merged_feature_nig_dict,
    ...
    def get_item_single_car_v2(self, selected_cav_base, ego_pose, isEgo=True):
        """
        ...
        if isEgo:
            lidar_np = mask_points_by_range(
                lidar_np, self.params["preprocess_q"]["cav_lidar_range"]
            )
            processed_lidar = self.pre_processor_q.preprocess(lidar_np)
        else:
            lidar_np = mask_points_by_range(
                lidar_np, self.params["preprocess_k"]["cav_lidar_range"]
            )
            processed_lidar = self.pre_processor_k.preprocess(lidar_np)
