...
def main():
    ...
    # print(hypes["model"]["pre_train_modules"])
    if (
        "pre_train_modules" in hypes["model"]
        and hypes["model"]["pre_train_modules"] is not None
    ):
        model = train_utils.load_pretrained_model(
            hypes["model"]["pre_train_modules"], model
        )
    ...
    torch.save(
        model_without_ddp.state_dict(),
        os.path.join(saved_path, "last.pth"),
    )
