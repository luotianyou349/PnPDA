...
def visualize_single_sample_output_gt(pred_tensor,
    ...
    def calculate_distance(box1, box2):
        """计算两个框之间的距离"""
        return torch.norm(box1 - box2, dim=-1)

    def filter_boxes(pred_boxes, gt_boxes, threshold=2):
        """过滤预测框，保留与gt框距离小于等于threshold的框"""
        device = pred_boxes.device
        filtered_boxes = []
        for pred_box in pred_boxes:
            min_distance = float('inf')
            for gt_box in gt_boxes:
                distances = calculate_distance(pred_box, gt_box)
                min_dist_gt = torch.max(distances)
                if min_dist_gt < min_distance:
                    min_distance = min_dist_gt.item()
            if min_distance <= threshold:
                filtered_boxes.append(pred_box)
        return torch.stack(filtered_boxes)

    # 过滤预测框
    pred_tensor = filter_boxes(pred_tensor, gt_tensor, threshold=10)

    # print("过滤后的预测框数量：", len(pred_tensor))
...
def visualize_single_sample_output_gt_v2(pred_tensor,
                                      gt_tensor,
                                      pcd,
                                      seg,
                                      show_vis=True,
    ...
    def custom_draw_geometry(pcd, seg, gt):
        vis = o3d.visualization.Visualizer()
        vis.create_window()
    ...
    def boxes_to_vertices(boxes):
        n = boxes.shape[0]
        vertices = torch.zeros((n, 8, 3), dtype=boxes.dtype, device=boxes.device)

        # 提取左上角和右下角坐标
        xmin, ymin, xmax, ymax = boxes.split(1, dim=1)

        # 计算顶点坐标
        vertices[:, 0, :] = torch.cat((xmin, ymin, torch.full_like(xmin, -2)), dim=1)
        vertices[:, 1, :] = torch.cat((xmax, ymin, torch.full_like(xmax, -2)), dim=1)
        vertices[:, 2, :] = torch.cat((xmax, ymax, torch.full_like(xmax, -2)), dim=1)
        vertices[:, 3, :] = torch.cat((xmin, ymax, torch.full_like(xmin, -2)), dim=1)
        vertices[:, 4, :] = torch.cat((xmin, ymin, torch.full_like(xmin, 0)), dim=1)
        vertices[:, 5, :] = torch.cat((xmin, ymin, torch.full_like(xmin, 0)), dim=1)
        vertices[:, 6, :] = torch.cat((xmax, ymin, torch.full_like(xmax, 0)), dim=1)
        vertices[:, 7, :] = torch.cat((xmax, ymax, torch.full_like(xmax, 0)), dim=1)

        return vertices

    # 过滤预测框
    pred_tensor = filter_boxes(pred_tensor, gt_tensor, threshold=10)
    seg_tensor = boxes_to_vertices(seg.squeeze(0))
    seg_tensor = filter_boxes(seg_tensor, gt_tensor ,threshold=10)

    print("过滤后的预测框数量：", len(seg_tensor))

    oabbs_pred = bbx2oabb(pred_tensor, color=(1, 0, 0))
    oabbs_gt = bbx2oabb(gt_tensor, color=(0, 1, 0))
    oabbs_seg = bbx2oabb(seg_tensor, color=(1, 0, 0))

    visualize_elements = [o3d_pcd] + oabbs_pred + oabbs_gt
    # exit(0)
    
    if show_vis:
        custom_draw_geometry(o3d_pcd, oabbs_seg, oabbs_gt)
    if save_path:
        save_o3d_visualization(visualize_elements, save_path)
