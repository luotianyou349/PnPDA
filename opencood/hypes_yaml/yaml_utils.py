...
def load_pp_pp_params(param):
    ...
    """
    cav_lidar_range = param['preprocess_q']['cav_lidar_range']
    cav_lidar_range_k = param['preprocess_k']['cav_lidar_range']
    voxel_size_q = param['preprocess_q']['args']['voxel_size']
    voxel_size_k = param['preprocess_k']['args']['voxel_size']


    grid_size_q = (np.array(cav_lidar_range[3:6]) - np.array(
        cav_lidar_range[0:3])) / \
                np.array(voxel_size_q)
    grid_size_q = np.round(grid_size_q).astype(np.int64)
    param['model']['args']['encoder_q']['args']['point_pillar_scatter']['grid_size'] = grid_size_q
    grid_size_k = (np.array(cav_lidar_range_k[3:6]) - np.array(
        cav_lidar_range_k[0:3])) / \
                np.array(voxel_size_k)
    grid_size_k = np.round(grid_size_k).astype(np.int64)
    print('grid_size_k',grid_size_k)
    param['model']['args']['encoder_k']['args']['point_pillar_scatter']['grid_size'] = grid_size_k
    ...
    param['model']['args']['encoder_k']['args']['W'] = math.ceil((cav_lidar_range_k[3] - cav_lidar_range_k[0]) / voxel_size_k[0])
    param['model']['args']['encoder_k']['args']['H'] = math.ceil((cav_lidar_range_k[4] - cav_lidar_range_k[1]) / voxel_size_k[1])
    param['model']['args']['encoder_k']['args']['D'] = math.ceil((cav_lidar_range_k[5] - cav_lidar_range_k[2]) / voxel_size_k[2])


    return param


def load_vn_pp_params(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
    ...
    grid_size_k = (np.array(cav_lidar_range[3:6]) - np.array(
        cav_lidar_range[0:3])) / \
                np.array(voxel_size_k)
    grid_size_k = np.round(grid_size_k).astype(np.int64)
    param['model']['args']['encoder_k']['args']['point_pillar_scatter']['grid_size'] = grid_size_k
    ...
    param['model']['args']['encoder_q']['args']['W'] = anchor_args['W']
    param['model']['args']['encoder_q']['args']['H'] = anchor_args['H']
    param['model']['args']['encoder_q']['args']['D'] = anchor_args['D']

    return param

def load_vn_vn_params(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
    ...
    param['model']['args']['encoder_k']['args']['W'] = math.ceil((cav_lidar_range_k[3] - cav_lidar_range_k[0]) / vw)
    param['model']['args']['encoder_k']['args']['H'] = math.ceil((cav_lidar_range_k[4] - cav_lidar_range_k[1]) / vh)
    param['model']['args']['encoder_k']['args']['D'] = math.ceil((cav_lidar_range_k[5] - cav_lidar_range_k[2]) / vd)

    return param


def load_pp_vn_params(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
...
def load_pp_sd_params(param):
    ...
    grid_size_k = (np.array(cav_lidar_range[3:6]) - np.array(
        cav_lidar_range[0:3])) / \
                np.array(voxel_size_k)
    grid_size_k = np.round(grid_size_k).astype(np.int64)
    param['model']['args']['encoder_k']['args']['grid_size'] = grid_size_k
    ...
    # sometimes we just want to visualize the data without implementing model
    param['model']['args']['encoder_k']['args']['W'] = math.ceil((cav_lidar_range[3] - cav_lidar_range[0]) / voxel_size_k[0])
    param['model']['args']['encoder_k']['args']['H'] = math.ceil((cav_lidar_range[4] - cav_lidar_range[1]) / voxel_size_k[1])
    param['model']['args']['encoder_k']['args']['D'] = math.ceil((cav_lidar_range[5] - cav_lidar_range[2]) / voxel_size_k[2])


    return param


def load_sd_vn_params(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
    ...
    grid_size_q = (np.array(cav_lidar_range[3:6]) - np.array(
        cav_lidar_range[0:3])) / \
                np.array(voxel_size_q)
    grid_size_q = np.round(grid_size_q).astype(np.int64)
    param['model']['args']['encoder_q']['args']['grid_size'] = grid_size_q
