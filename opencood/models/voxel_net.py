...
class RPN(nn.Module):
    def __init__(self, anchor_num=2):
        ...
        self.block_3 = [Conv2d(128, 256, 3, 1, 1)]
        self.block_3 += [nn.Conv2d(256, 256, 3, 1, 1) for _ in range(5)]
        self.block_3 = nn.Sequential(*self.block_3)

        self.deconv_1 = nn.Sequential(nn.ConvTranspose2d(256, 256, 2, 2, 0),
                                      nn.BatchNorm2d(256))
    ...
    def forward(self, x):
        ...
        x_skip_2 = x
        x = self.block_3(x)
        # print(x.shape,x_skip_1.shape,x_skip_2.shape)
        x_0 = self.deconv_1(x)
...
class VoxelNet(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        out=vwfs.view(self.N, -1, self.H, self.W)
        print('out',out.shape)
        psm, rm = self.rpn(out)
